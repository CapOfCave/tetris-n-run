...
public class GraphicalTools {
	...
	private static int getRGB(int red, int green, int blue, int alpha, double factor) {
		int r = (int) (red * factor / 255);
		int g = (int) (green * factor / 255);
		int b = (int) (blue * factor / 255);
...
//		return rgb;
	...
	public static BufferedImage setBrightness(BufferedImage image, int factor) {
		BufferedImage img = new BufferedImage(image.getWidth(), image.getHeight(), BufferedImage.TYPE_INT_ARGB);
		...
		for (int i = 0; i < image.getWidth(); i++) {
			for (int j = 0; j < image.getHeight(); j++) {
				...
				int blue = (current) & 0x0ff;
				int alpha = (current >> 24) & 0x0ff;
				img.setRGB(i, j, getRGB(red, green, blue, alpha, factor));
			}
		}
